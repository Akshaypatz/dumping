(function (global) {
  ...
  DCCSDK.prototype._createModal = function (details) {
    ...
    merchantInfo.innerHTML = `
      ...
      <div>
        <div style="font-weight:600;font-size:16px;color:#1f2937;">akshaylabs</div>
        <div style="font-size:13px;color:#6b7280;">Order #696969</div>
      </div>
    `;
    ...
    details.currencies.forEach(curr => {
      ...
      if (curr.code !== details.merchantHomeCurrency) {
        ...
        disclaimerBox.innerHTML = `
          <div style="display:flex;justify-content:space-between;margin-bottom:4px;font-weight:500;">
            ...
            <span>1 ${details.merchantHomeCurrency} = ${details.exchangeRate} ${curr.code}</span>
          </div>
...
footer.style.cssText = `
  ...
  return footer;
    ...
    /* ---------- TIMER ---------- */
    const expiry = new Date(details.expiryTime).getTime();
    const updateTimer = () => {
  ...
  /* ---------- FLOW ---------- */
  DCCSDK.prototype.initiateDCCFlow = function (dccOrderId, dccTokenId) {
    return new Promise(async (resolve, reject) => {
      try {
        const details = await mockGetDccDetails(dccOrderId, dccTokenId);
  ...
  async function getDccDetails(orderId, tokenId, oToken) {
    console.log("getDccDetails called with", orderId, tokenId);

    try {
        const response = await fetch(`https://your-api-endpoint.com/dcc-details`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'OToken': oToken // your auth token header
            },
            body: JSON.stringify({
                orderId: orderId,
                tokenId: tokenId
